    ...
    public class CustomMembershipProvider : ExtendedMembershipProvider
    {
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            using (var db = new FuelCardDBEntities())
            {
                ...
                return Convert.ToBoolean(db.Staffs.Any(x => x.Staff_Email.ToLower() == username.ToLower() && x.Staff_Password == passhash && x.IsActive == true && x.IsDeleted==false));
            }
        }
        #endregion
        ...
        public Staff GetUserDetails(string username)
        {
            using (var db = new FuelCardDBEntities())
            {
                return db.Staffs.FirstOrDefault(x => (x.Staff_Email.ToLower() == username.ToLower()));
            }
        ...
        public override void CreateOrUpdateOAuthAccount(string provider, string providerUserId, string userName)
        {
            ...
            using (var db = new FuelCardDBEntities())
            {
                var userProfile = db.Staffs.Where(x => x.Staff_Email.Equals(userName));
                if (userProfile == null)
                {
        ...
        public class MembershipHelper
        {
            ...
            public static string CreatSalt(int size)
            {
                try
                {
                    ...
                    provider.GetBytes(data); //Fills this array of bytes with cryptographically strong sequence of random values
